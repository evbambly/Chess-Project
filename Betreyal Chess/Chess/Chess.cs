...
namespace Chess
{
    ...
    public class Chess
    {
        ...
        private const int neutralSpawns = 3;
        ...
        public Chess()
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                for (int j = 0; j < 8; j++)
                {
                    Color color = i < 5 ? Color.white : Color.black;
                    if (i > 1 && i < 6)
            ...
            for (int i = 0; i < neutralSpawns; i++)
            {
                Random random = new Random();
                int x = random.Next(2, 6);
                int y = random.Next(8);
                int piece = random.Next(4);
                switch (piece)
                {
                    case 0:
                        board[x, y] = new Bishop(Color.neutral);
                        break;
                    case 1:
                        board[x, y] = new Knight(Color.neutral);
                        break;
                    case 2:
                        board[x, y] = new Queen(Color.neutral);
                        break;
                    case 3:
                        board[x, y] = new Rook(Color.neutral);
                        break;
                }
            }
        ...
        public void Play()
        {
            while (gameState == GameState.Play)
            {
                ...
                gameState = MoveCheck.IsGameCheckMate(board, toTile, !whiteTurn);
        ...
        public static Piece[,] AmendBoard(Piece[,] board, Tile fromTile, Tile toTile)
        {
            ...
            if (fromTile.x != toTile.x || fromTile.y != toTile.y)
            {
                ...
                if (board[fromTile.x, fromTile.y] is King king)
                {
                    int yDistance = toTile.y - fromTile.y;
                    if (yDistance == 2)
                    {
                        board[toTile.x, 5] = new Rook(king.color);
                        board[toTile.x, 7] = new EmptyTile();
                    }
                    if (yDistance == -2)
                    {
                        board[toTile.x, 3] = new Rook(king.color);
        ...
        private Piece CrownPawn(bool whiteTurn)
        {
            ...
            while (chosenCrown is null)
            {
                ...
                {
                    case ConsoleKey.B:
                        chosenCrown = new Bishop(whiteTurn? Color.white : Color.white);
                        break;
                    case ConsoleKey.K:
                        chosenCrown = new Knight(whiteTurn ? Color.white : Color.white);
                        break;
                    case ConsoleKey.Q:
                        chosenCrown = new Queen(whiteTurn ? Color.white : Color.white);
                        break;
                    case ConsoleKey.R:
                        chosenCrown = new Rook(whiteTurn ? Color.white : Color.white);
                        break;
                }
            }
            return chosenCrown;
        ...
        private Dictionary<char[,], int> AmendBoardStatesCount()
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                for (int j = 0; j < 8; j++)
                {
                    thisBoardState[i, j] = board[i,j].color == Color.white ? board[i, j].ToString().ToUpper()[0] : board[i,j].ToString()[0];
                }
            }
                        ...
                        else if (i > 0 && j > 0 && (i + j) % 2 == 0)
                        {
                            ...
                            else
                            {
                                ...
                                if (piece.color == Color.white)
                                    middle =  " w";
                                if (piece.color == Color.neutral)
                                    middle = " n";
                                middle += piece.ToString()[0]+" ";
                            }
                        }
                        else if (k == 1)
                        {
        ...
        public static List<Tile> NewFindPlayerPieces(Piece[,] board, Color player)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                for (int j = 0; j < 8; j++)
                {
                    if (board[i, j].color == player && !(board[i, j] is EmptyTile))
                    {
        ...
        public static Tile FindKing(Piece[,] board, bool whitePlayer)
        {
            ...
            List<Tile> playerPieces = NewFindPlayerPieces(board, whitePlayer? Color.white: Color.black);
