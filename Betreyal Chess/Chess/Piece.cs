...
namespace Chess
{
    ...
    {
        public EmptyTile() : base(Color.neutral) { }
            ...
            */
            ...
            if (xDistance == 0 && yDistance == 2 && !hasMoved)
            {
                ...
                if (board[rookPos.x, rookPos.y] is Rook rook && !rook.HasMoved)
                {
                    ...
                    {
                        if (MoveCheck.IsPlayerInCheck(board, fromTile, currentTile, color == Color.white ? true : false))
                        {
                            moveIsLegal = false;
                        }
    ...
    public class Pawn : Piece
    {
        ...
        {
            EnPassant = false;
            initialPosition = color == Color.white ? 1 : 6;
            xAdvance = color == Color.white ? 1 : -1;
        }
        public override bool IsMoveLegal(Tile fromTile, Tile toTile)
        {
        ...
        public override bool SecondMove(Piece[,] board, Tile fromTile, Tile toTile)
        {
            ...
            if (!moveIsLegal && yDistance == 1 && xDistance == xAdvance)
            {
                ...
                {
                    moveIsLegal = pawn.EnPassant && pawn.color != this.color ? true : false;
                }
            }
            return moveIsLegal;
        }
