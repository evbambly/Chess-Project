...
namespace Chess
{
    ...
    {
        public static GameState IsGameStaleMate(Piece[,] board, bool whiteTurn, int movestoStaleMate, Dictionary<char[,], int> boardStatesCount)
        {
            /*
            ...
            List<Tile> playerPieces = Chess.NewFindPlayerPieces(board, whiteTurn);
            List<Tile> foePieces = Chess.NewFindPlayerPieces(board, !whiteTurn);
            //If both players don't have enough material to force a checkmate 
            //(King || King + Knight || King + Bishop) -> It is a stalemate
            if (!staleMate && playerPieces.Count < 3 && foePieces.Count < 3)
            {
        ...
        public static GameState IsGameCheckMate(Piece[,] board, Tile landedOnTile)
        {
            /* CheckMate occurs if the following are true:
             * The player is in Check
             * The player has no legal move to exit the Check
             */
            bool whiteTurn = board[landedOnTile.x, landedOnTile.y].whitePiece;
            bool checkMate = false;
            if (IsPlayerInCheck(board, landedOnTile, landedOnTile))
            {
                checkMate = true;
                List<Tile> playerPieces = Chess.NewFindPlayerPieces(board, whiteTurn);
                foreach (Tile piece in playerPieces)
        ...
        public static bool IsMovePossible(Piece[,] board, bool whiteTurn, Tile fromTile, Tile toTile)
        {
            ...
            if (IsMoveSane(whiteTurn, chosenPiece, board[toTile.x, toTile.y]))
            {
                if (IsMoveUnobstructed(board, fromTile, toTile))
                {
                    ...
                    {
                        moveIsPossible = IsPlayerInCheck(board, fromTile, toTile) ? false : true;
                    }
                }
            }
        ...
        private static bool IsMoveSane(bool whiteTurn, Piece fromPiece, Piece toPiece)
        {
            ...
            bool moveIsSane = fromPiece.whitePiece == whiteTurn && !(fromPiece is EmptyTile);
            return moveIsSane && (toPiece.whitePiece != whiteTurn || toPiece is EmptyTile);
        }
        public static bool IsPlayerInCheck(Piece[,] importedBoard, Tile fromTile, Tile toTile)
        {
